...
function fixImgInMarkdownLink(mdx) {
  return mdx.replace(
    ...
    (_match, imgAttrs) => {
      ...
      const srcMatch = /src\s*=\s*"(.*?)"|src\s*=\s*'(.*?)'|src\s*=\s*([^\s"'>]+)/i.exec(imgAttrs);
      const altMatch = /alt\s*=\s*"(.*?)"|alt\s*=\s*'(.*?)'|alt\s*=\s*([^\s"'>]+)/i.exec(imgAttrs);

      const src = (srcMatch && (srcMatch[1] || srcMatch[2] || srcMatch[3])) || '';
      const alt = (altMatch && (altMatch[1] || altMatch[2] || altMatch[3])) || 'image';
...
function protectBareIdentifiers(text) {
  ...
  return (
    text
      ...
      .replace(/&#123;\s*ipgTransactionId\s*&#125;/gi, '&#123;ipgTransactionId&#125;') // keep as entities
      // raw braces → replace with literal text
      .replace(/\{(\s*)ipgTransactionId(\s*)\}/gi, '&#123;$1ipgTransactionId$2&#125;')
  );
}
...
function protectPathParams(text) {
  ...
  return text.replace(/\{([A-Za-z][A-Za-z0-9_-]*)\}/g, (_m, name) => `&#123;${name}&#125;`);
}
...
function escapeAngleBracketEmails(s) {
  return s.replace(/<([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,})>/g, '&lt;$1&gt;');
}
...
function convertCustomTabMdToMdx(md) {
  ...
  while (true) {
    ...
    const isStartTab = /\btype\s*:\s*tab\b/i.test(startInner) && /\btitles\s*:/i.test(startInner);
    ...
    while (true) {
      ...
      const isMiddleTab = /\btype\s*:\s*tab\b/i.test(inner) && !/\btitles\s*:/i.test(inner);
      if (isMiddleTab) {
    ...
    if (endStart === -1) {
      result += md.slice(startMatch.index, startMatch.index + startMatch[0].length);
      cursor = startMatch.index + startMatch[0].length;
      continue;
    }
    ...
    for (const t of titles) {
      lines.push(`    <TabsTrigger value="${escapeAttr(t)}">${escapeText(t)}</TabsTrigger>`);
    }
    lines.push(`  </TabsList>`);
...
function preprocessMdxToMarkdown(raw, productName) {
  ...
  // 🔒 Emails like <legalpapers@fiserv.com>

# API links

classic UI [full path link](/product/DeveloperStudioTest/product/DeveloperStudioTest/api/put/cards&branch=develop&version=11.0.1)

classic UI [incorrect full path link](/api/cards?type=put&version=11.0.1)

classic UI [relative link](/product/DeveloperStudioTest/api/put/cards&version=11.0.1)

new UI [relative link](../../api/put/cards?version=11.0.1)

new UI [full path link](/product/DeveloperStudioTest/api/post/cards?version=11.0.1)

new UI [different product](/product/CommerceHub/api/post/payments-vas/v1/3ds/authenticate)

new UI [incorrect full path](/api/get/cards?version=11.0.1)

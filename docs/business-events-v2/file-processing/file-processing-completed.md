# Business Event: File Processing Completed

## Overview

The File Processing service domain publishes `File_Processing_Completed` as part of the `File_System_Lifecycle` event family. The publication informs downstream consumers that the File processing completed business activity occurred in VisionNext.

**Standard:** BIAN

**Service Domain:** File Processing

**BusinessEventName:** `File_Processing_Completed`

**BusinessEventGroup:** `File_System_Lifecycle`

## Purpose

Publish the File processing completed event so downstream consumers can react to the completion represented by the source event.

## Business Trigger

FILE PUBLISH REJECTED FILE PUBLISH PARTIAL
...
File Processing publishes `File_Processing_Completed` when the documented trigger occurs: fILE PUBLISH REJECTED FILE PUBLISH PARTIAL.
...
| Trigger | FILE PUBLISH REJECTED FILE PUBLISH PARTIAL |
| Business Purpose | Publish the File processing completed event so downstream consumers can react to the completion represented by the source event. |

### Business Consumers

| Consumer | Subscription Purpose |
| --- | --- |
| File processing and operations support teams | Support operational monitoring and exception follow-up for `File_Processing_Completed`. |
| Clearing and settlement processing services | Consume `File_Processing_Completed` to keep local business state aligned with the source event. |
...
| Event data hub, analytics, and operational monitoring consumers | Support reporting, operational metrics, and trend analysis for `File_Processing_Completed`. |

### Business Outcome

The event establishes an authoritative enterprise record for the `File_Processing_Completed` business occurrence. Subscribing systems use it to keep servicing, operations, analytics, audit, and customer-facing views aligned with the source event state.
...
`File_Processing_Completed` notifies subscribers that the File processing completed business activity occurred in VisionNext.
...
| `BusinessEventName` | string | Fixed business event name used by subscribers and routing configuration. | File_Processing_Completed |
...
| Field | Type | Description | Sample |
| --- | --- | --- | --- |
| `FileId` | integer | Identifier for the related file id in the File processing completed event. | 1001 |
| `Source` | string | Event-specific source value supplied by the file system event. | VISA |
| `FileStatus` | string | Status value represented by file status. | Closed |
| `ElapsedTimeInMinutes` | integer | Date or timestamp associated with elapsed time in minutes for this event. | 0 |

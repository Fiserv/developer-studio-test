# Business Event: File Processing Start

## Overview

The File Processing service domain publishes `File_Processing_Start` as part of the `File_System_Lifecycle` event family. The publication informs downstream consumers that the File processing start business activity occurred in VisionNext.

**Standard:** BIAN

**Service Domain:** File Processing

**BusinessEventName:** `File_Processing_Start`

**BusinessEventGroup:** `File_System_Lifecycle`

## Purpose

Publish the File processing start event so downstream consumers can react to the business state change represented by the source event.

## Business Trigger

The event is generated when VisionNext records the File processing start business activity in the File Processing domain.
...
File Processing publishes `File_Processing_Start` as part of the `File_System_Lifecycle` event family. The event is generated when VisionNext records the File processing start business activity in the File Processing domain.
...
| Trigger | The event is generated when VisionNext records the File processing start business activity in the File Processing domain. |
| Business Purpose | Publish the File processing start event so downstream consumers can react to the business state change represented by the source event. |

### Business Consumers

| Consumer | Subscription Purpose |
| --- | --- |
| File processing and operations support teams | Support operational monitoring and exception follow-up for `File_Processing_Start`. |
| Clearing and settlement processing services | Consume `File_Processing_Start` to keep local business state aligned with the source event. |
...
| Event data hub, analytics, and operational monitoring consumers | Support reporting, operational metrics, and trend analysis for `File_Processing_Start`. |

### Business Outcome

The event establishes an authoritative enterprise record for the `File_Processing_Start` business occurrence. Subscribing systems use it to keep servicing, operations, analytics, audit, and customer-facing views aligned with the source event state.
...
`File_Processing_Start` notifies subscribers that the File processing start business activity occurred in VisionNext.
...
| `BusinessEventName` | string | Fixed business event name used by subscribers and routing configuration. | File_Processing_Start |
...
| Field | Type | Description | Sample |
| --- | --- | --- | --- |
| `FileId` | integer | Identifier for the related file id in the File processing start event. | 1001 |

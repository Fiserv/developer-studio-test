# Business Event: File Hash Total Check Failed

## Overview

The File Processing service domain publishes `File hash total check failed` as part of the `Transaction_Lifecycle` event family. The publication informs downstream consumers that the File hash total check failed business activity occurred in VisionNext.

**Standard:** BIAN

**Service Domain:** File Processing

**BusinessEventName:** `File hash total check failed`

**BusinessEventGroup:** `Transaction_Lifecycle`

## Purpose

Publish the File hash total check failed event so downstream consumers can react to the business state change represented by the source event.

## Business Trigger

The event is generated when VisionNext records the File hash total check failed business activity in the File Processing domain.
...
File Processing publishes `File hash total check failed` as part of the `Transaction_Lifecycle` event family. The event is generated when VisionNext records the File hash total check failed business activity in the File Processing domain.
...
| Event | `File hash total check failed` |
| Event Group | `Transaction_Lifecycle` |
| Trigger | The event is generated when VisionNext records the File hash total check failed business activity in the File Processing domain. |
| Business Purpose | Publish the File hash total check failed event so downstream consumers can react to the business state change represented by the source event. |

### Business Consumers

| Consumer | Subscription Purpose |
| --- | --- |
| File processing and operations support teams | Support operational monitoring and exception follow-up for `File hash total check failed`. |
| Clearing and settlement processing services | Consume `File hash total check failed` to keep local business state aligned with the source event. |
...
| Event data hub, analytics, and operational monitoring consumers | Support reporting, operational metrics, and trend analysis for `File hash total check failed`. |

### Business Outcome

The event establishes an authoritative enterprise record for the `File hash total check failed` business occurrence. Subscribing systems use it to keep servicing, operations, analytics, audit, and customer-facing views aligned with the source event state.
...
`File hash total check failed` notifies subscribers that the File hash total check failed business activity occurred in VisionNext.
...
| `BusinessEventName` | string | Fixed business event name used by subscribers and routing configuration. | File hash total check failed |
...
| `EventBody.FileId` | integer | Identifier for the related file id in the File hash total check failed event. | 1001 |
